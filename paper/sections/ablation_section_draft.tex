...
\input{tables/table3_components.tex}
...
We evaluate sensitivity to $\lambda$, the regularization coefficient in the recursive least-squares solve, over $\lambda \in \{0.01, 0.1, 0.5\}$. Across all nine configurations the largest spread in AIA is $0.39$ points, and in five of nine it is below $0.2$. The analytic projection is therefore insensitive to $\lambda$ over more than an order of magnitude, and needs no per-dataset tuning.
