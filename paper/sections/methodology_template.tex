...
\STATE $\Sigma \gets \mathbf{0}_{D \times D}$, \quad $\Phi \gets \mathbf{0}_{D \times p}$, \quad $\mathcal{G} \gets \emptyset$, \quad $\mathcal{B} \gets \emptyset$
\STATE
...
\FOR{each incoming batch $(X, Y)$ in the stream}
    ...
    \IF{end of block}
        ...
        \STATE Solve $(\Sigma + \gamma I)\, W^* = \Phi$ \COMMENT{ridge $\gamma = 10^{-3}$}
